...
function init_grid_data() {
    $.ajax({
        ...
        data: { 'sql': "select formatDateTime(toDateTime(CAST(timestamp as DateTime64)), '%Y-%m-%d %H:%M:%S') as time, index from IndexRealTime order by timestamp desc limit 100;" },
        dataType: "json",
        success: function (result) {
            if (result.status == 0) {
                $("#jsGrid1").jsGrid({
    ...
    $.ajax({
        ...
        async: false,
        url: '/picker/get_sql_data/',
        data: { 'sql': "select short_name, year, format('{0}%',cast(ratio as String)) as weight from EachYearAllRatio order by year desc, ratio desc limit 100;" },
        dataType: "json",
        success: function (result) {
            if (result.status == 0) {
                $("#jsGrid2").jsGrid({
...
$(function () {
    var item = $("a[href='/pipeline/pindex/']");
