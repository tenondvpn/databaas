...
function get_table_detail(node) {
    ...
    $.ajax({
        type: 'post',
        url: '/pyplugin/get_table_detail/',
        dataType: 'json',
        async: false,
        data: { 'table': node.text, 'db': parent.text },
        success: function (result) {
            if (result.status == 0) {
...
function update_new_proc() {
    ...
    if (prc_type == 3) {
        var python = $("#proc_python").val();
        python = python.replace(new RegExp("", "gm"), '<br/>');
        python = python.replace(new RegExp(" ", "gm"), '&nbsp;');
        document.getElementById('proc_template').innerHTML = python;
    }
...
function autoFormatSelectionMain() {
    var value = pythonFormatter.format(codemirror_editor.getValue().trim(), {
        language: "sql",
        uppercase: false,
    });
...
function createTaskAjax() {
    ...
    var param = "name=Python计算" + getRandomInt(1000);
    param += "&type=1";
...
function show_table_info(open, result) {
    if (open) {
        ...
        var value = pythonFormatter.format(result.create.trim(), {
            language: "python",
            uppercase: false,
        });
...
function init_codemirror(python) {
    ...
    codemirror_editor.setValue(python);
    $("#top_python_data").show();
    $("#buttom_python_data").show();
}

function CallSql() {
    console.log(codemirror_editor.getValue().trim());
    if (codemirror_editor.getValue().trim() == '') {
        return;
    }
    ...
    $.ajax({
        type: 'post',
        async: true,
        url: '/pyplugin/get_python_data/',
        data: { 'python': codemirror_editor.getValue().trim() },
        dataType: "json",
        success: function (result) {
            if (result.status == 0) {
                $("#jsGrid1").jsGrid({
