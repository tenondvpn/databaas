...
function show_auth() {
    ...
    var url = "/picker/chain_block/1/";
    $("#modal_iframe iframe").attr('src', url);
    $("#modal_iframe").attr('type', 1);
...
function show_block_jsplumb_init() {
    ...
    jsPlumbToolkit.ready(function () {
        ...
        console.log("get block hash: " + $('#block_hash').val());
        show_block_get_blocks($('#block_hash').val());
    });
}

//获取该流程的所有任务
function show_block_get_blocks(table_addr) {
    $.ajax({
        type: 'post',
        url: '/picker/get_blocks/',
        async: false,
        ...
        success: function (result) {
            if (result.status) {
                ...
                var task_list = result.fileds;
                console.log("max_block_hash: " + max_block_hash + ", task_list[0].id: " + task_list[0].id);
                if (max_block_hash == task_list[0].id) {
                    return;
                }

                max_block_hash = task_list[0].id;
                toolkit.clear();
                for (var i = 0; i < task_list.length; ++i) {
                    toolkit.addNode({
                        "id": task_list[i].id,
                        "name": "0x" + task_list[i].hash.substring(0, 20) + "\n        " + task_list[i].timestamp,
                        "w": 180,
                        "h": 30,
                    });
                }
...
function do_transaction() {
    var password = "b5b3128c236fcec044c303b54d55a97e20bf98b625fec1de6a2a0fffcd8c7cf7";
    ...
    var data = create_tx(self_private_key, self_public_key,  $('#block_hash').val(), 1000, 10000, 1);
    $.ajax({
        type: 'post',
        async: true,
        url: 'http://82.156.224.174:8783/transaction',
        data: data,
        dataType: "json"
